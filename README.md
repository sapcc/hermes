...
If neither is specified, then events from the `default` index will be returned (useful for debugging only, to see what events are not being assigned to an index).
